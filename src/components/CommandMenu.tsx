...
export default function CommandMenu({ searchData }: { searchData: SearchEntry[] }) {
  ...
  const filtered = searchData.filter((seed) => {
        ...
        <div className="overflow-y-auto p-2 bg-neutral-900">
          {filtered.length === 0 ? (
            ...
            filtered.map((seed) => (
              <button
                key={seed.slug}
                onClick={() => handleSelect(seed.slug)}
              ...
              >
                <span
                  className={`text-[9px] uppercase font-mono px-2 py-1 rounded border opacity-70 group-hover:opacity-100 w-16 text-center ${
                    seed.format === 'code'
                      ? 'text-green-400 border-green-900 bg-green-900/10'
                      : seed.format === 'poetry'
                        ? 'text-pink-400 border-pink-900 bg-pink-900/10'
                        : seed.format === 'course'
                ...
                <div className="flex-1">
                  ...
                  <div className="text-neutral-600 text-[10px] truncate w-64 md:w-96 font-mono">
                    {seed.pillar} // {seed.tags.join(', ')}
                  </div>
                </div>
